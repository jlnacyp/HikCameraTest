...
int HIKCamera::connect()
{
    ...
    strcpy(struLoginInfo.sDeviceAddress, "192.168.2.64"); //设备IP地址
    struLoginInfo.wPort = 8000; //设备服务端口
    strcpy(struLoginInfo.sUserName, "admin"); //设备登录用户名
    strcpy(struLoginInfo.sPassword, "Lz123456"); //设备登录密码
    ...
    // 设置连接相机的信息
    struLoginInfo2.bUseAsynLogin = 0; //同步登录方式
    strcpy(struLoginInfo2.sDeviceAddress, "192.168.2.164"); //设备IP地址
    struLoginInfo2.wPort = 8000; //设备服务端口
    strcpy(struLoginInfo2.sUserName, "admin"); //设备登录用户名
    strcpy(struLoginInfo2.sPassword, "LJ123456"); //设备登录密码

    //设备信息, 输出参数
    lUserID2 = NET_DVR_Login_V40(&struLoginInfo2, &struDeviceInfoV402);
...
int HIKCamera::startStreaming2(HWND hPlayWnd)
{
    if (!isConn)
    {
        qDebug() << u8"相机未连接";
    ...
    struPlayInfo2.hPlayWnd = hPlayWnd; //需要SDK解码时句柄设为有效值，仅取流不解码时可设为空
    ...
    lRealPlayHandle2 = NET_DVR_RealPlay_V40(lUserID2, &struPlayInfo2, NULL, NULL); //海康摄像头实时回放设置
...
//{
//    std::vector<char> inputData(3840*2160*4 + 54);
//    DWORD size= 0;
//    DWORD* psize = &size;
